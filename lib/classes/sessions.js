...
Sessions.prototype.authenticateUser = function(req, res, next) {
    ...
    if (typeof req.session.user !== 'undefined') {
        self.logger.log('debug','active session',req.session.user);
        next();
    }else{
        ...
        try{
            username = self.decrypt( cookie, self.sessionSecret ) ;
            self.logger.log('debug','decrypt auth cookie',username);
            self.connection.query('select ID, Email, Password, Salt, PasswordEncryption from `Member` where Email = ?',[username], function(err, rows, fields) {
                if (err){
                    self.logger.log('error','Querying user by cookie variable. '+JSON.stringify(err));
                    next();
                }else{
                    if ( rows.length == 1 ){
                        req.session.user = rows[0].Email;
                        self.logger.log('debug','recreate session by cookie');
                    }else{
                        self.logger.log('warning','auth cookie found, but user is not valid');
                    }
                    next();
                }
            });
...
Sessions.prototype.sessionsLocals  = function(req, res, next) {
    var self=this;
    
    if ( (typeof req.session!= 'undefined') &&  (req.session!=null) && (typeof req.session.user == 'string')){
        res.locals.CurrentUser = {
            Email: req.session.user
        };
    }else if (typeof res.locals.CurrentUser !== 'undefined'){
...
// `logout()`
Sessions.prototype.logout = function(req, res, next){
    var self=this;
    if ((typeof req.query!='undefined') && (typeof req.query.logout!='undefined') && (req.query.logout == '1')){
        self.logger.log('debug','clear session');
        res.clearCookie(self.authenticateCookieName);
        req.session.destroy();
        req.session = null;
    }
    next();

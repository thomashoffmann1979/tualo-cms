...
// * `listStop` defaults to `</ul>`, set the closing html tag for the list (or sublist).
        ...
        if (typeof options.linkBasePath == 'undefined'){
// * `linkBasePath` defaults to `.`.
            options.linkBasePath = '.';
        }
        if (typeof options.linkStart == 'undefined'){
// * `linkStart` defaults to `.`.
            options.linkStart = '<a href="{url}">';
        }
        if (typeof options.linkStop == 'undefined'){
// * `linkStop` defaults to `.`.
            options.linkStop = '</a>';
        }
        if (typeof options.noLinks == 'undefined'){
// * `noLinks` defaults to `false`.
            options.noLinks = false;
        }
        for( var i in tree ){
            item = tree[i];
            if ((options.inMenues === false) || (item.ShowInMenus == 1)){
                noptions = options;
                noptions.linkBasePath += '/' + tree[i].URLSegment;


                if (options.noLinks===true){
                    text = tree[i].Title
                }else{
                    linkStart = noptions.linkStart.replace('{url}',noptions.linkBasePath);
                    text = linkStart + tree[i].Title + noptions.linkStop;
                }
                
                if (typeof tree[i].tree == 'undefined'){
                    result.push(text);
                }else{
                    result.push(text + treeHTML(tree[i].tree,noptions));
                }
            }
        }
